                        ...
                        <div class="flex justify-end">
                            <div class="w-64 space-y-2">
                                <div class="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                                    <span>Subtotal:</span>
                                    <span id="subtotal">{{ number_format($quote->items->sum(fn($i) => $i->quantity * $i->unit_price), 2) }}</span>
                                </div>
                                <div class="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                                    <span>Tax (<span id="tax-rate-display">{{ $quote->tax_rate }}</span>%):</span>
                                    <span id="tax-amount">{{ number_format($quote->items->sum(fn($i) => $i->quantity * $i->unit_price) * $quote->tax_rate / 100, 2) }}</span>
                                </div>
                                <div class="flex justify-between text-base font-semibold text-gray-900 dark:text-white border-t border-gray-200 dark:border-gray-700 pt-2">
                                    <span>Total:</span>
                                    <span id="grand-total">{{ number_format($quote->total, 2) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
